...
bool LexicalBlock::strEqual(const wchar_t* a, const wchar_t* b) {
    if (a == b) return true;
    if (!a || !b) return false;
	while (*a && *b) {
        if (*a != *b) return false;
...
void LexicalBlock::strCopy(wchar_t* dest, const wchar_t* src) {
    if (!dest || !src) return;
    while (*src) {
        *dest = *src;
        dest++; src++;
...
int LexicalBlock::strLen(const wchar_t* str) {
    int len = 0;
    while (str && str[len]) len++;
    return len;
}

wchar_t* LexicalBlock::strDup(const wchar_t* src) {
    if (!src) return 0;
    int len = strLen(src);
...
void LexicalBlock::intToStr(int value, wchar_t* buffer) {
    if (value == 0) {
		buffer[0] = L'0';
		buffer[1] = L'\0';
		return;
...
bool LexicalBlock::isDigit(wchar_t c) {
	return c >= L'0' && c <= L'9';
}

bool LexicalBlock::isLetter(wchar_t c) {
	return (c >= L'a' && c <= L'z') || (c >= L'A' && c <= L'Z') ||
		   (c >= L'А' && c <= L'Я') || (c >= L'а' && c <= L'я');
}

bool LexicalBlock::isWhitespace(wchar_t c) {
	return c == L' ' || c == L'\t' || c == L'\n' || c == L'\r';
}
...
LexicalBlock::LexicalBlock() {
	keywords[0] = L"ЕСЛИ";
	keywords[1] = L"ИНАЧЕ";
	keywords[2] = L"КОНЕЦ_ЕСЛИ";
	keywords[3] = L"ПОКА";
	keywords[4] = L"КОНЕЦ_ПОКА";
    keywordCount = 5;
...
int LexicalBlock::findIdentifier(const wchar_t* name) {
    for (int i = 0; i < identifierCount; i++) {
...
int LexicalBlock::addIdentifier(const wchar_t* name) {
...
void LexicalBlock::analyze(const wchar_t* sourceCode) {
    clear();
	...
	while (sourceCode[i] != L'\0') {
        ...
        for (int d = 0; d < delimiterCount; d++) {
            const wchar_t* delim = delimiters[d];
            int j = 0;
			while (delim[j] != L'\0' && sourceCode[i + j] == delim[j]) {
				j++;
			}
            if (delim[j] == L'\0') {
        ...
        if (isDigit(sourceCode[i])) {
            ...
            while (isDigit(sourceCode[i])) {
                value = value * 10 + (sourceCode[i] -L'0');
                i++;
            }
		...
		if (isLetter(sourceCode[i]) || sourceCode[i] ==L'_') {
            int start = i;
			while (isLetter(sourceCode[i]) || isDigit(sourceCode[i]) || sourceCode[i] == L'_') {
                i++;
            }
